...
| Variable | Description |
|---|---|
| `NEXT_PUBLIC_SUPABASE_URL` | Supabase project URL (preferred) |
| `NEXT_PUBLIC_SUPABASE_ANON_KEY` | Supabase anon/public key (preferred) |
| `SUPABASE_URL` | Optional fallback for hosted env injection |
| `SUPABASE_PUBLISHABLE_KEY` | Optional fallback public key |
| `NEXT_PUBLIC_API_URL` | Backend API URL (default: http://localhost:4000) |
| `NEXT_PUBLIC_SITE_URL` | Optional canonical web URL for auth email redirects (default: current browser origin) |
...
- Create two Vercel projects from the same repo:
  - `donasaurs-api` with **Root Directory** = `apps/api`
  - `donasaurs-web` with **Root Directory** = `apps/web`
- The `build` scripts in both apps build `@donasaurs/domain` from the monorepo root first, so `@donasaurs/domain` resolves without changing root to `..`.
